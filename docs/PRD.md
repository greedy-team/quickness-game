...
|------|------|
| **메커닉** | 방향키로 받기/피하기 |
| **아이템** | 진짜 기억(받기) / 가짜 기억(피하기) |
| **점수 (다단계, 가변)** | 받은 위치 정확도별 점수 차등 부여 (다구간) |
| **소요시간** | 약 30초 |

### 🪞 STAGE 4 — 3분할 동시 (4번 문, 최종)
| 항목 | 상세 |
|------|------|
| **메커닉** | 화면 3분할, Stage 1·2·3 메커닉 동시 진행 |
| **점수 (다단계, 가변)** | 각 화면별 다단계 점수 합산 |
...
```
[오프닝 15초] → [허브] → [1·2·3 자유 진행] → [4번 자동 개방] → [최종전 30초] → [엔딩 10초]
```
...
#### **STAGE 2 — 빈 교실**
```
First-person POV looking into dark empty Korean classroom 
...
#### **STAGE 3 — 무너지는 방**
```
First-person POV in a surreal Korean classroom with ceiling 
...
#### **가짜 그린이 실루엣**
```
Silhouette of Korean female high school student in uniform 
standing still, long black hair covering face, transparent 
PNG background, full body, slightly tilted head, eerie 
posture --ar 1:2
```
...
#### **거울 속 세 그린이 (Stage 4)**
```
Three identical Korean female high school students in 
uniform standing side by side, reflected in cracked mirrors, 
each with subtly different expressions, transparent PNG 
background --ar 16:9
```
...
1) torn uniform photo with faces scratched out, 2) notebook 
with red scribbles, 3) photo with one face replaced. 
Glitchy red aura, transparent PNG background --ar 1:1
```

#### **오프닝 컷씬**
```
Korean female high school student standing alone in dark 
empty hallway at night, looking at her phone showing a photo 
of herself she doesn't remember taking, cold blue lighting, 
cinematic horror, third-person rear shot --ar 16:9
```

#### **엔딩 — 성공**
```
First-person POV of cracked mirror showing only one 
reflection, dawn light breaking through window, dust 
particles, hopeful yet eerie, soft warm glow mixing with 
cold blue --ar 16:9
```

#### **엔딩 — 실패**
```
First-person POV of mirror showing reflection that smiles 
when you don't, dark red atmosphere, distorted edges, deep 
horror --ar 16:9
```
